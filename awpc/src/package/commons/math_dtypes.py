...
from exceptions import InvalidInput, MissingParameters
...
def _validateFloat(n: float) -> float:
    try:
        ...
        return toReturn
def _isClose(a: float, b: float, epsilon: float | None = EPSILON):
...
class SquareMatrix:
    ...
    def __neg__(self) -> SquareMatrix:
        return SquareMatrix([[(self[idx1][idx2]) * -1 for idx1 in range(self._dim)] for idx2 in range(self._dim)])
    ...
    def setrow(self, idx: int, newrow: list) -> None:
        self[idx] = newrow
    def row(self, idx: int) -> list:
        return self[idx]
    def setcolumn(self, idx: int, newcolumn: list) -> None:
    ...
    def identity(Size: int) -> SquareMatrix:
        """Default Matrix constructor that returns the identity matrix of the given size."""
    ...
    def _T(M: SquareMatrix) -> SquareMatrix:
        transposed = SquareMatrix(size=M._dim)
    ...
    def dim(self) -> int:
        """
        Returns the dimension of the matrix. Since it's square, only one integer is needed, compared to a rectangular one.
        """
        return self._dim
    @property
    def trace(self) -> float:
        """
        Returns the trace of the matrix. The trace is defined as the sum of all the elements on the diagonal, e. g. `x_00`, `y_11`, `z_22`, etc.
        """
        return sum(self[idx][idx] for idx in range(len(self._data)))
    @property
    def T(self) -> SquareMatrix:
        """
        Returns the transposed matrix of itself. A transposed matrix is the original matrix but with its rows and columns swapped. 
    ...
    def is_diagonal(self) -> bool:
        def _matrixsum(M: SquareMatrix):
            return sum([sum([M[idx1][idx2] for idx1 in range(M._dim)]) for idx2 in range(M._dim)])
        
        return True if _matrixsum(self) == self.trace else False
